...
# iterate over a range of dimensions to compute the number of proofs needed to
# achieve the target FLP soundness error, because the optimal choice of
# wraparound check parameters doesn't change with dimension.
...
def search_num_wr_successes(target_soundness_bits, num_wr_checks):
    ...
    '''
    # Perform a binary search of `num_wr_successes` in `[0, num_wr_checks + 1)`.
    # Note `min_num_wr_successes` is an inclusive lower bound, and
    # `max_num_wr_successes` is an exclusive upper bound.
    min_num_wr_successes = 0
    max_num_wr_successes = num_wr_checks + 1
    num_wr_successes = None
    while min_num_wr_successes != max_num_wr_successes:
        mid_num_wr_successes = min_num_wr_successes + \
            (max_num_wr_successes - min_num_wr_successes) // 2
        prob = wr_sound(num_wr_checks, mid_num_wr_successes)
        if bits_of_security(prob) < target_soundness_bits - 1:
            # Doesn't satisfy soundness, increase the lower bound.
            min_num_wr_successes = mid_num_wr_successes + 1
        else:
            # Satisfies soundness, record this `mid_num_wr_successes` and
            # decrease the upper bound.
            num_wr_successes = mid_num_wr_successes
            max_num_wr_successes = mid_num_wr_successes
    if num_wr_successes is None:
        raise ValueError("The number of wraparound checks is too small to "
                         "satisfy soundness error requirement.")
    return num_wr_successes
...
def search_wr_params(target_soundness_bits,
    ...
    wr_and_user_params = []
    for (l2_norm_bound, num_frac_bits, _, field) in user_params:
...
def search_vdaf_params(target_soundness_bits, wr_and_user_params, dimensions):
    '''
    ...
    number of proofs needed to satisfy the FLP soundness error for each
    dimension, and return the number of proofs with each combination of
    `wr_and_user_params` and `dimensions`, as the full VDAF parameters.
    '''
    vdaf_params = []
    for (num_wr_checks,
         num_wr_successes,
        ...
        for dimension in dimensions:
            ...
            vdaf_params.append((l2_norm_bound,
                                ...
                                zk_error,
                                overall_sound))
    print("Displaying full VDAF parameters:")
    return vdaf_params
...
# the range of possible floating point values, and satisfy the aggregation
# requirement.
user_params = []
...
l2_norm_bound = 1
dimension = 1000
for field in [Field48, Field56, Field64, Field128]:
    for num_frac_bits in [15, 20, 24]:
        try:
            ...
            print("Failed to initialize PineValid due to: {}".format(str(e)))
            continue
...
# Dimension determines how many proofs we need to run with the FLP to achieve
# the desired FLP soundness error, so we will check each of the dimensions for
# each set of parameters in `user_params`.
dimensions = [10**i for i in range(3, 6)]
...
# `user_params`, with `dimension` being hardcoded. Then for each feasible set of
# wraparound check and user parameters, we replace `dimension` with a range of
# `dimensions` to compute the number of proofs required to achieve the desired
# FLP soundness error.
for (target_bits,
     max_num_wr_checks,
     num_wr_checks_step) in [(100, 1500, 50),
                             (32, 80, 5),
                             (64, 500, 20),
                             (80, 1000, 30)]:
    ...
    # error, for each combination of `dimensions` and `wr_and_user_params`.
